...
async function getBlockWinnerAccountId(url, isBHD, height, customEndpoint = 'burst') {
  let accountId = await getBlockWinnerAccountIdOrNull(url, isBHD, height, customEndpoint);
  let retries = 0;
  while (accountId === null && retries < 24) {
    await new Promise(resolve => setTimeout(resolve, 5 * 1000));
    accountId = await getBlockWinnerAccountIdOrNull(url, isBHD, height, customEndpoint);
    retries += 1;
  }
...
async function getBlockWinnerAccountIdOrNull(url, isBHD, height, customEndpoint = 'burst') {
  try {
    if (isBHD) {
